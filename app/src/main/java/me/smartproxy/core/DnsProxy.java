...
public class DnsProxy implements Runnable {
    ...
    private static final ConcurrentHashMap<Integer, String> IPDomainMaps
            = new ConcurrentHashMap<>();

    private static final ConcurrentHashMap<String, Integer> DomainIPMaps
            = new ConcurrentHashMap<>();

    private static final long QUERY_TIMEOUT_NS = 10 * 1000000000L;
    ...
    public void run() {
        try {
            ...
            DatagramPacket packet = new DatagramPacket(RECEIVE_BUFFER, 28,
                    RECEIVE_BUFFER.length - 28);
    ...
    private void tamperDnsResponse(byte[] rawPacket, DnsPacket dnsPacket, int newIP) {
        ...
        ResourcePointer rPointer = new ResourcePointer(rawPacket,
                question.Offset() + question.Length());
    ...
    private void OnDnsResponseReceived(IPHeader ipHeader, UDPHeader udpHeader,
            DnsPacket dnsPacket) {
    ...
    private boolean interceptDns(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        ...
        if (question.Type == 1) {
            if (ProxyConfig.getInstance().needProxy(question.Domain,
                    getIPFromCache(question.Domain))) {
                int fakeIP = getOrCreateFakeIP(question.Domain);
    ...
    public void onDnsRequestReceived(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        if (!interceptDns(ipHeader, udpHeader, dnsPacket)) {
            ...
            InetSocketAddress remoteAddress = new InetSocketAddress(
                    CommonMethods.ipIntToInet4Address(state.RemoteIP), state.RemotePort);
            DatagramPacket packet = new DatagramPacket(udpHeader.m_Data, udpHeader.m_Offset + 8,
                    dnsPacket.Size);
