                ...
                for (int i = 0; i < m_ProxyList.size(); i++) {
                    try {
                        ...
                        InetAddress address = InetAddress
                                .getByName(config.ServerAddress.getHostName());
                        if (address != null && !address.equals(config.ServerAddress.getAddress())) {
                            config.ServerAddress = new InetSocketAddress(address,
                                    config.ServerAddress.getPort());
                        }
                    } catch (Exception e) {
                    }
                }
        ...
        for (String line : lines) {
            ...
            try {
                ...
                throw new Exception(String.format(
                        "SmartProxy config file parse error: line:%d, tag:%s, error:%s", lineNumber,
                        tagString, e));
            }
    ...
    private void tryAddProxy(String[] lines) {
        for (String line : lines) {
            ...
            while (m.find()) {
                ...
                config.ServerAddress = new InetSocketAddress(m.group(1),
...
//            String proxyString = "ss://dGFibGU6U0hhcnJ5MzNAaGltZXNzaWFoLm1lOjgzODg=";
    ...
    private boolean convertToBool(String valueString) {
        ...
        if (valueString.equals("on") || valueString.equals("1") || valueString.equals("true")
