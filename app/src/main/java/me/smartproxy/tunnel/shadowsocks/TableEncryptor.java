...
public class TableEncryptor implements IEncryptor {
    ...
    private byte[] mergeSort(byte[] srcArray, long a) {
        ...
        int stepSize, leftOffset, leftMaxOffset, rightOffset, rightMaxOffset, dstOffset, leftValue,
                rightValue;
                ...
                for (dstOffset = 0; dstOffset < 256; ) {
                    ...
                    for (; dstOffset < rightMaxOffset; dstOffset++) {
                        if (rightOffset == rightMaxOffset) {
                            ...
                            boolean isLeftSmallThanRight = a > 0 ? ((a % leftValue - a % rightValue)
                                    <= 0) : (((a1 % leftValue + a2 % leftValue + 1) % leftValue
                                    - (a1 % rightValue + a2 % rightValue + 1) % rightValue) <= 0);
